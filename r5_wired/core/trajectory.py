...
class TrajectoryKernel:
    ...
    def __init__(self, epsilon: float = 0.01, policy=None) -> None:
        """
        WIRING NOTE (Sprint 12):
          If `policy` (a tenir_governance.PolicyEngine) is provided, epsilon
          is taken from policy.epsilon (canonical value 1e-6). The explicit
          epsilon arg is kept for backward compatibility with R5 code that
          constructs TrajectoryKernel(epsilon=0.01) directly.
        """
