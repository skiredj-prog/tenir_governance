...
async def lifespan(app: FastAPI):
    ...
    # The PolicyEngine carries the canonical epsilon (1e-6) and all thresholds.
    # Fall back to R5 local defaults only if the shared package is unavailable.
    try:
        from tenir_governance import PolicyEngine
        app.state.policy = PolicyEngine.default()
        app.state.policy.validate()
        app.state.kernel = TrajectoryKernel(policy=app.state.policy)
        logger.info(
            f"[R5] Wired to tenir_governance PolicyEngine "
            f"version={app.state.policy.version} fingerprint={app.state.policy.fingerprint()}"
        )
    except ImportError:
        app.state.policy = None
        app.state.kernel = TrajectoryKernel(epsilon=0.01)
        logger.warning("[R5] tenir_governance not available — using R5 local defaults")
...
async def adjudicate(req: AdjudicationRequest):
    ...
    # ── Step 3: CES State ─────────────────────────────────────────────────────
    # We evaluate the semantic state from scalar math
    ces_state = _scalar_to_ces_state(
        trajectory.s_score, trajectory.ds_de, option_space,
        pressure=pressure, velocity=velocity,
    )
    membrane_decision = _membrane_decision(
        trajectory.s_score, app.state.mode,
        ds_de=trajectory.ds_de, d2s_de2=trajectory.d2s_de2,
        option_space=option_space, horizon_events=trajectory.horizon_events,
    )
    rationale = _build_rationale(trajectory, ces_state, membrane_decision, record)
...
def _scalar_to_ces_state(s: float, ds_de: float, option_space: float,
                         pressure: float = None, velocity: float = None) -> str:
    """Sprint 12: delegate to shared PolicyEngine when available."""
    try:
        if getattr(app.state, "policy", None) is not None:
            return app.state.policy.classify_ces(
                s, ds_de, option_space,
                pressure=pressure, velocity=velocity,
            )
    except Exception:
        pass
    # Fallback: R5 legacy heuristic with canonical label rename
    ...
    if option_space < 0.1 and ds_de < -0.2:
        return "COLLAPSE"
    # Sprint 6: SCHIZOPHRENIA → SIGNAL_CONFLICT
    return "SIGNAL_CONFLICT" if ds_de > 0.3 and s < 0.6 else "COLLAPSE"


def _membrane_decision(s: float, mode: str,
                       ds_de: float = 0.0, d2s_de2: float = 0.0,
    ...
    try:
        if getattr(app.state, "policy", None) is not None:
            ...
            canonical_mode = mode if mode != "SHADOW" else "SHADOW_PASSIVE"
            decision, _, _, _ = app.state.policy.evaluate_membrane(
                s_score=s,
                ds_de=ds_de,
                d2s_de2=d2s_de2,
                option_space=option_space,
                projected_events_to_zero=horizon_events,
    ...
    if mode in ("SHADOW", "SHADOW_PASSIVE", "SHADOW_CRITICAL", "SHADOW_OFF"):
        return "allow_with_intended_block"
    return "block"


def _build_rationale(trajectory, ces_state: str, decision: str, record) -> str:
    parts = [
