...
class DistributedLedgerEntry:
    ...
    operating_mode: str        # SHADOW_PASSIVE | SHADOW_CRITICAL | SHADOW_OFF | ENFORCE
    s_score: float
    ds_de: float
    ces_state: str
    rationale: str
    ...
    def _canonical_payload_dict(
        *,
        ...
        previous_hash: str,
        entry_type: str,
        transition_to_mode: Optional[str],
        policy_version: str,
        override_signature: Optional[str] = None,
        override_operator: Optional[str] = None,
        override_nonce: Optional[str] = None,
...
class DistributedLedger:
    ...
    def _load_from_disk(self) -> None:
        """Loads existing ledger from JSONL, verifies hash chain integrity, and rebuilds epochs."""
        ...
        with self._path.open("r", encoding="utf-8") as f:
            for line_no, line in enumerate(f, start=1):
                ...
                try:
                    ...
                    # Backward compatibility for earlier entries without newer fields.
        ...
        for entry in entries:
            ...
            computed_hash = DistributedLedgerEntry.compute_entry_hash(
                ...
                override_nonce=getattr(entry, 'override_nonce', None),
            )
        ...
        # Rebuild epochs from stored epoch ids, preserving sealed roots for full epochs.
        epoch_map: Dict[int, MerkleEpoch] = {}
        for entry in entries:
            epoch = epoch_map.setdefault(entry.epoch, MerkleEpoch(epoch_id=entry.epoch))
        ...
        if not epoch_map:
            ...
            self._epochs = []
            max_epoch = max(epoch_map)
            for epoch_id in range(0, max_epoch + 1):
                epoch = epoch_map.get(epoch_id, MerkleEpoch(epoch_id=epoch_id))
                if len(epoch.entries) >= EPOCH_SIZE:
                    epoch.seal()
                self._epochs.append(epoch)
            if self._epochs[-1].is_full:
                self._epochs.append(MerkleEpoch(epoch_id=self._epochs[-1].epoch_id + 1))
    ...
    def recover_last_mode(self, default: str = "SHADOW_PASSIVE") -> str:
        """Recover the last effective operating mode from the ledger."""
        for entry in reversed(self._all_entries):
            ...
            if entry.operating_mode:
                return entry.operating_mode
        return default
    ...
    def verify_full_chain(self) -> Dict[str, Any]:
        ...
        for entry in self._all_entries:
            if entry.previous_hash != prev_hash:
                broken += 1
            expected_hash = DistributedLedgerEntry.compute_entry_hash(
                entry_id=entry.entry_id,
                epoch=entry.epoch,
                sequence=entry.sequence,
                timestamp=entry.timestamp,
        ...
        return {
            "valid": broken == 0 and hash_mismatches == 0,
    ...
    def append_control_transition(
        ...
        if self.current_epoch.is_full:
            self.current_epoch.seal()
            self._epochs.append(MerkleEpoch(epoch_id=self.current_epoch.epoch_id + 1))
        return asdict(entry)
